"""scenetextqa — Scene text VQA dataset.

Registry key: scenetextqa

Source: https://github.com/abd-abdur/SceneTextVQA (or similar)
HF mirror: SpencerPao/SceneTextVQA

When the answer field is a list, the first element is used.

Download:
    from img_2_svg_pretraining.training.training_core.datasets.public.scenetextqa import download
    download("/data/scenetextqa")
"""
import os
...
def download(data_path: str, n_procs: int = 1) -> None:
    """Download SceneTextVQA to *data_path* via the HuggingFace Hub."""
    import datasets as hf_datasets

    try:
        ds = hf_datasets.load_dataset("SpencerPao/SceneTextVQA", trust_remote_code=True)
        ds.save_to_disk(data_path)
        print(f"[scenetextqa] Saved to {data_path}")
    except Exception as exc:
        raise RuntimeError(
            "SceneTextVQA is not available via SpencerPao/SceneTextVQA. "
...
def get_source(
    ...
    raw_answer = source.get("answer") or ""
...
def get_scenetextqa_dataargs(
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(
            ...
            f"Run img_2_svg_pretraining.training.training_core.datasets.public.scenetextqa.download(data_path) first."
        )
