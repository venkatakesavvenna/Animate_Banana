...
from img_2_svg_pretraining.training.training_core.datasets.layout.utils import get_paragraph_order, visualise_promptable_dataset
...
DATASET_PATH=os.path.expanduser("/root/.cache/huggingface/CompHRDoc/")
DATASET_REGISTRY_NAME = "comp_hr"
mapping = {}
LAYOUT_CLASSES=[]
...
def convert_data_to_huggingface(path: str = DATASET_PATH):
    global mapping, LAYOUT_CLASSES
    train_records = []
    missing_regions = 0

    # data['images'][0]   -> {'id': 1, 'file_name': '1401.3699_0.png', 'height': 842, 'width': 596}
    # data['annotations'] -> {'category_id': 1, 'iscrowd': 0, 'id': 11, 'image_id': 2,
    #                          'bbox': [113, 94, 359, 135], 'segmentation': [...], 'area': 48465}
    # data['categories']  -> [{'id': 1, 'name': 'fig', ...}, ...]
    json_path = os.path.join(path, "Comp-HRDoc/HRDH_MSRA_POD_TEST/coco_test.json")
    ...
    # Populate LAYOUT_CLASSES from category list
    for cat in data["categories"]:
        if cat["name"] not in LAYOUT_CLASSES:
            LAYOUT_CLASSES.append(cat["name"])
    ...
    for image_id, img_info in tqdm(id_to_image.items()):
        try:
            # '1401.3699_0.png' -> HRDH/images/1401.3699/0.png
            file_name = img_info["file_name"]
            stem = os.path.splitext(file_name)[0]           # '1401.3699_0'
            last_underscore = stem.rfind("_")
            paper_id = stem[:last_underscore]               # '1401.3699'
            page_num = stem[last_underscore + 1:]           # '0'
            image_path = os.path.join(path, "HRDH", "images", paper_id, page_num + ".png")
            ...
            print(f"Error processing image {img_info.get('file_name', image_id)}: {e}")
            missing_regions += 1
            continue
...
def get_comp_hr_dataargs(get_sam_source_fn, debug_path="", seed=42, sam_image_size=1024, data_path = DATASET_PATH, train=True):
    ds = convert_data_to_huggingface(data_path)
