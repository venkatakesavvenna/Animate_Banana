...
DATASET_PATH=os.path.expanduser("/root/.cache/huggingface/hub/datasets--opendatalab--OmniDocBench/snapshots/91fe284bbfacfa687959ae3eb00846ca852aa907")
DATASET_REGISTRY_NAME = "omnidocs"
mapping = {}
LAYOUT_CLASSES=[]
...
def convert_promptable_to_huggingface(path: str = DATASET_PATH):
    ...
    path = os.path.join(path, "OmniDocBench.json")
    with open(path, "r") as f:
        data = json.load(f)

    for item in tqdm(data):
        try:
            image_path = os.path.join(DATASET_PATH, "images", item["page_info"]["image_path"])
            annotations = []
            for ann in item["layout_dets"]:
                category_name = ann["category_type"]
                if category_name =="abandon":
                    continue
                if category_name not in LAYOUT_CLASSES:
                    LAYOUT_CLASSES.append(category_name)
                bbox = ann["poly"]
                annotations.append({
                    "category": category_name,
                    "bbox": bbox
            ...
            print(f"Error processing item {item['page_info']['image_path']}: {e}")
            missing_regions += 1
            continue
