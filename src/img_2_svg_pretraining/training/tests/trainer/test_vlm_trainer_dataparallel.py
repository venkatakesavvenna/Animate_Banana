...
@pytest.mark.trainer
@pytest.mark.gpu
@pytest.mark.dataparallel
@pytest.mark.external
@pytest.mark.parametrize(
    ("spec", "attn_implementation"),
    _dataparallel_cases(),
    ids=[f"{spec.slug}-{backend}" for spec, backend in _dataparallel_cases()],
)
def test_vlm_sam_custom_trainer_runs_two_gpu_dataparallel_step(
    spec,
    attn_implementation,
    qwen25_model_id,
    gemma3_model_id,
    ...
    if not runtime_enabled("dataparallel"):
        pytest.skip("DataParallel trainer tests disabled")
    ...
    env["TRAINING_TEST_TRAINER_MODE"] = "dataparallel"
    env["TRAINING_TEST_TRAINER_DATASET_SIZE"] = "2"
    env["TRAINING_TEST_TRAINER_ATTN"] = attn_implementation
    env[trainer_backend_env_var(spec, "DP")] = attn_implementation
    ...
    assert "DP_RESULT=passed" in result.stdout
    assert "DP_N_GPU=2" in result.stdout
    assert "DP_GLOBAL_STEP=1" in result.stdout
