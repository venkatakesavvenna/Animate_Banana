...
#SBATCH --job-name=training_molmo_pretrain_mn
#SBATCH --nodes=2
#SBATCH --nodelist=ip-10-20-213-4,ip-10-20-205-50
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=64
#SBATCH --mem-per-cpu=4G
#SBATCH --exclusive
#SBATCH --output=../src/logs/slurm_molmo_%j.out
#SBATCH --error=../src/logs/slurm_molmo_%j.err
...
TRAINING_CONFIG="${TRAINING_CONFIG:-/code/src/img_2_svg_pretraining/training/configs/mn_molmo_pretrain.yaml}"
...
# Capture git info on the host (git is available here, not inside the container).
# GIT_DIRTY counts uncommitted changes; 0 = clean tree.
...
torchrun \
  ...
  2>&1 | tee logs/molmo_pretrain_mn_rank_\${NODE_RANK}.log
"
'

echo "Molmo pre-training job completed."
