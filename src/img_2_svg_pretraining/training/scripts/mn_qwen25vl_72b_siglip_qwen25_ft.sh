...
#SBATCH --job-name=mn_qwen25vl_72b_siglip_qwen25_ft
#SBATCH --nodes=4
#SBATCH --nodelist=ip-10-20-227-40,ip-10-20-227-97,ip-10-20-205-50,ip-10-20-208-160
#SBATCH --ntasks-per-node=1
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=64
#SBATCH --mem-per-cpu=4G
#SBATCH --exclusive
#SBATCH --output=/fsxvision_new/venkat.kesav/img_2_svg_pretraining/src/img_2_svg_pretraining/training/logs/slurm_mn_qwen25vl_72b_siglip_qwen25_ft_%j.out
#SBATCH --error=/fsxvision_new/venkat.kesav/img_2_svg_pretraining/src/img_2_svg_pretraining/training/logs/slurm_mn_qwen25vl_72b_siglip_qwen25_ft_%j.err
...
TRAINING_CONFIG="${TRAINING_CONFIG:-/code/src/img_2_svg_pretraining/training/configs/mn_qwen25vl_72b_siglip_qwen25_ft.yaml}"
...
if ! docker exec '${CONTAINER_NAME}' bash -lc \"test -f '${MANIFEST_CACHE}/dataset_info.json'\"; then
    echo '[Preflight] Manifest not found — building.'
    docker exec \
        -e DATA_PATH='${DATA_PATH}' \
        -e MANIFEST_CACHE='${MANIFEST_CACHE}' \
        '${CONTAINER_NAME}' \
    ...
    echo '[Preflight] Manifest already exists.'
fi
"
...
torchrun \
  ...
  2>&1 | tee logs/mn_qwen25vl_72b_siglip_qwen25_ft_rank_\${NODE_RANK}.log
"
'

echo "Job mn_qwen25vl_72b_siglip_qwen25_ft completed."
