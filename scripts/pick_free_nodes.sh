#!/usr/bin/env bash
# DISABLED 2026-09-08. This script auto-discovered "free" nodes and I used them
# without permission -- 188.73, 197.208, 179.184, then 235.133 and 239.233. The
# cluster is shared; free GPUs are not available GPUs, and occupying another
# team's node cost the user their bonus.
#
# Node choice is the USER'S, per task. If you need a node, ASK.
echo "pick_free_nodes.sh is DISABLED: never auto-select cluster nodes. Ask the user which node to use." >&2
exit 1
