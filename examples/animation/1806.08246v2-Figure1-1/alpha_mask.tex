\documentclass{standalone}
\usepackage{tikz}
\usepackage{animate}
\usetikzlibrary{fit, positioning, arrows.meta, shapes.geometric, calc}

\begin{document}
\begin{animateinline}[controls, loop]{1}
\multiframe{17}{iFrame=1+1}{
\begin{tikzpicture}[
    >=Stealth,
    % Parent Block Background Styles
    pink_bg/.style={fill=pink!50!white, draw=black!80, line width=1.2pt},
    blue_bg/.style={fill=blue!15!white, draw=black!80, line width=1.2pt},
    yellow_bg/.style={fill=orange!15!white, draw=black!80, line width=1.2pt},
    green_bg/.style={fill=green!10!white, draw=black!80, line width=1.2pt},
    % Inner Block Styles
    inner_box_blue/.style={fill=blue!25!white, draw=black!80, line width=0.8pt, minimum width=6cm, minimum height=0.9cm, align=center, font=\sffamily},
    inner_box_green/.style={fill=white, draw=black!80, line width=0.8pt, minimum width=3.8cm, minimum height=0.9cm, align=center, font=\sffamily},
    % Arrow Styles
    red_arrow/.style={draw=red!80!black, line width=1.5pt, -{Stealth[scale=1.2]}},
    blue_arrow/.style={draw=blue!80!black, line width=1.5pt, -{Stealth[scale=1.2]}},
    orange_arrow/.style={draw=orange!90!black, line width=1.5pt, -{Stealth[scale=1.2]}},
    green_arrow/.style={draw=green!60!black, line width=1.5pt, -{Stealth[scale=1.2]}},
    gray_arrow/.style={draw=black!60, line width=2pt, -{Stealth[scale=1.2]}},
    % Placeholder Style
    placeholder_node/.style={draw, dashed, fill=gray!10, align=center, font=\sffamily}
]

    % Explicit Spacious Bounding Box
    \path [use as bounding box] (-4, -2) rectangle (24, 16);

    % Opacity Logic Definitions
    % MaxFrame is 17 (16 steps + 1 finale)
    \pgfmathsetmacro{\MaxFrame}{17}
    
    % Step 1: ff_box (Container). Children appear in steps 2,3. Must be visible 1-3 and 17.
    \pgfmathsetmacro{\opFfBox}{(\iFrame>=1 && \iFrame<=3) || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 2: data_group (Child of 1).
    \pgfmathsetmacro{\opData}{\iFrame==2 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 3: cnn_group (Child of 1).
    \pgfmathsetmacro{\opCnn}{\iFrame==3 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 4: pd_box (Container). Children appear in steps 5,6,7,8. Must be visible 4-8 and 17.
    \pgfmathsetmacro{\opPdBox}{(\iFrame>=4 && \iFrame<=8) || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 5: globe_group (Child of 2 - mapped to PD box context).
    \pgfmathsetmacro{\opGlobe}{\iFrame==5 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 6: pd_b1.
    \pgfmathsetmacro{\opPdOne}{\iFrame==6 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 7: pd_b2.
    \pgfmathsetmacro{\opPdTwo}{\iFrame==7 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 8: pd_b3.
    \pgfmathsetmacro{\opPdThree}{\iFrame==8 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 9: ia_box (Container). Children appear in steps 10,11. Must be visible 9-11 and 17.
    \pgfmathsetmacro{\opIaBox}{(\iFrame>=9 && \iFrame<=11) || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 10: ia_logo_group.
    \pgfmathsetmacro{\opIaLogo}{\iFrame==10 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 11: web_group.
    \pgfmathsetmacro{\opWeb}{\iFrame==11 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 12: pi_box (Container). Children appear in steps 13,14,15,16. Must be visible 12-16 and 17.
    \pgfmathsetmacro{\opPiBox}{(\iFrame>=12 && \iFrame<=16) || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 13: pi_b1.
    \pgfmathsetmacro{\opPiOne}{\iFrame==13 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 14: pi_b2.
    \pgfmathsetmacro{\opPiTwo}{\iFrame==14 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 15: pi_b3.
    \pgfmathsetmacro{\opPiThree}{\iFrame==15 || \iFrame==\MaxFrame ? 1 : 0.15}
    
    % Step 16: pi_b4.
    \pgfmathsetmacro{\opPiFour}{\iFrame==16 || \iFrame==\MaxFrame ? 1 : 0.15}

    % =========================================================
    % 1. BACKGROUND BOXES
    % =========================================================
    
    % Facial Feature Extractor Bounds
    \coordinate (ff_tl) at (0, 14.5);
    \coordinate (ff_br) at (8.5, 8.5);
    \begin{scope}[opacity=\opFfBox, transparency group]
        \node[fit=(ff_tl) (ff_br), pink_bg] (ff_box) {};
        \node[font=\Large\sffamily\bfseries] at (4.25, 13.7) {Facial Feature Extractor};
    \end{scope}

    % Person Dictionary Bounds
    \coordinate (pd_tl) at (11.8, 15.0);
    \coordinate (pd_br) at (20.3, 7.0);
    \begin{scope}[opacity=\opPdBox, transparency group]
        \node[fit=(pd_tl) (pd_br), blue_bg] (pd_box) {};
        \node[font=\Large\sffamily\bfseries] at (16.05, 14.2) {Person Dictionary};
    \end{scope}

    % Internet Archive Bounds
    \coordinate (ia_tl) at (0, 6.0);
    \coordinate (ia_br) at (8.5, -0.5);
    \begin{scope}[opacity=\opIaBox, transparency group]
        \node[fit=(ia_tl) (ia_br), yellow_bg] (ia_box) {};
        \node[font=\Large\sffamily\bfseries] at (4.25, 5.2) {Internet Archive};
    \end{scope}

    % Person Identification Bounds
    \coordinate (pi_tl) at (10.0, 6.0);
    \coordinate (pi_br) at (20.3, -0.5);
    \begin{scope}[opacity=\opPiBox, transparency group]
        \node[fit=(pi_tl) (pi_br), green_bg] (pi_box) {};
        \node[font=\Large\sffamily\bfseries] at (15.15, 5.2) {Person Identification};
    \end{scope}

    % =========================================================
    % 2. BLOCK CONTENTS
    % =========================================================

    % --- A. Facial Feature Extractor ---
    
    % Data Cylinder (Step 2)
    \begin{scope}[opacity=\opData, transparency group]
        \coordinate (data_tl) at (0.5, 12.0);
        \coordinate (data_br) at (3.5, 9.2);
        \draw[fill=red!70!black, thick] (1.0, 9.5) arc (180:360:1cm and 0.3cm) -- (3.0, 9.8) arc (360:180:1cm and 0.3cm) -- cycle;
        \draw[fill=pink!80!red, thick] (1.0, 9.8) arc (180:360:1cm and 0.3cm) -- (3.0, 10.2) arc (360:180:1cm and 0.3cm) -- cycle;
        \draw[fill=gray!15, thick] (1.0, 10.2) arc (180:360:1cm and 0.3cm) -- (3.0, 11.5) arc (360:180:1cm and 0.3cm) -- cycle;
        \draw[fill=gray!15, thick] (2.0, 11.5) ellipse (1cm and 0.3cm);
        \node[font=\large\sffamily] at (2.0, 10.8) {Data};
        \node[fit=(data_tl) (data_br)] (data_group) {};
    \end{scope}

    % CNN Block (Step 3)
    \begin{scope}[opacity=\opCnn, transparency group]
        \coordinate (cnn_tl) at (4.6, 12.2);
        \coordinate (cnn_br) at (7.8, 9.4);
        \fill[black!30] (4.8, 9.4) rectangle (7.7, 11.4);
        \draw[fill=white, thick] (4.7, 9.5) rectangle (7.6, 11.5);
        \foreach \yA in {10.0, 10.5, 11.0} {
            \foreach \yB in {9.8, 10.3, 10.8, 11.3} { \draw[gray, thin] (4.95, \yA) -- (5.75, \yB); }
        }
        \foreach \yA in {9.8, 10.3, 10.8, 11.3} {
            \foreach \yB in {10.0, 10.5, 11.0} { \draw[gray, thin] (5.75, \yA) -- (6.55, \yB); }
        }
        \foreach \yA in {10.0, 10.5, 11.0} {
            \foreach \yB in {10.2, 10.8} { \draw[gray, thin] (6.55, \yA) -- (7.35, \yB); }
        }
        \foreach \y in {10.0, 10.5, 11.0} { \node[circle, draw=black, fill=red!70!black, inner sep=1.8pt] at (4.95, \y) {}; }
        \foreach \y in {9.8, 10.3, 10.8, 11.3} { \node[circle, draw=black, fill=white, inner sep=1.8pt] at (5.75, \y) {}; }
        \foreach \y in {10.0, 10.5, 11.0} { \node[circle, draw=black, fill=white, inner sep=1.8pt] at (6.55, \y) {}; }
        \foreach \y in {10.2, 10.8} { \node[circle, draw=black, fill=yellow, inner sep=1.8pt] at (7.35, \y) {}; }
        \node[font=\sffamily] at (6.15, 12.0) {CNN Training};
        \node[fit=(cnn_tl) (cnn_br)] (cnn_group) {};
    \end{scope}

    % --- B. Person Dictionary ---

    % Globe Icon (Step 5)
    \begin{scope}[opacity=\opGlobe, transparency group]
        \coordinate (globe_tl) at (12.0, 13.8);
        \coordinate (globe_br) at (13.4, 12.4);
        \draw[thick, fill=white] (12.7, 13.1) circle (0.55);
        \draw[thick] (12.7, 13.1) ellipse (0.25 and 0.55);
        \draw[thick] (12.15, 13.1) -- (13.25, 13.1);
        \draw[thick] (12.23, 13.35) -- (13.17, 13.35);
        \draw[thick] (12.23, 12.85) -- (13.17, 12.85);
        \node[fit=(globe_tl) (globe_br)] (globe_group) {};
    \end{scope}

    % Internal Text Nodes PD
    \begin{scope}[opacity=\opPdOne, transparency group]
        \node[inner_box_blue] (pd_b1) at (16.05, 12.6) {Select entities \& \\ gather exemplary images};
    \end{scope}
    
    \begin{scope}[opacity=\opPdTwo, transparency group]
        \node[inner_box_blue] (pd_b2) at (16.05, 10.5) {Feature extraction};
    \end{scope}
    
    \begin{scope}[opacity=\opPdThree, transparency group]
        \node[inner_box_blue] (pd_b3) at (16.05, 8.4) {Filtering};
    \end{scope}

    % PD Internal Arrows
    \begin{scope}[opacity=\opPdTwo, transparency group]
        \draw[blue_arrow] (pd_b1) -- (pd_b2);
    \end{scope}
    \begin{scope}[opacity=\opPdThree, transparency group]
        \draw[blue_arrow] (pd_b2) -- (pd_b3);
    \end{scope}

    % --- C. Internet Archive ---

    % Internet Archive Logo (Step 10)
    \begin{scope}[opacity=\opIaLogo, transparency group]
        \coordinate (ia_logo_tl) at (0.0, 4.8);
        \coordinate (ia_logo_br) at (3.8, 0.8);
        \fill[black] (0.8, 1.2) rectangle (3.4, 1.4);
        \fill[black] (0.9, 1.4) rectangle (3.3, 1.6);
        \fill[black] (1.0, 1.6) rectangle (3.2, 1.8);
        \foreach \x in {1.20, 1.56, 1.92, 2.28, 2.64, 3.00} {
            \fill[black] (\x-0.08, 1.8) rectangle (\x+0.08, 3.5);
        }
        \fill[black] (0.9, 3.5) rectangle (3.3, 3.7);
        \fill[black] (1.0, 3.7) -- (3.2, 3.7) -- (2.1, 4.4) -- cycle;
        \node[rotate=90, font=\small\sffamily, tracking=5pt] at (0.3, 2.7) {INTERNET};
        \node[font=\small\sffamily, tracking=5pt] at (2.1, 4.8) {A R C H I V E};
        \node[fit=(ia_logo_tl) (ia_logo_br)] (ia_logo_group) {};
    \end{scope}

    % Web Content Windows (Step 11)
    \begin{scope}[opacity=\opWeb, transparency group]
        \coordinate (web_tl) at (4.2, 4.6);
        \coordinate (web_br) at (8.0, 0.8);
        \fill[gray!20, draw=black!70, thick] (5.8, 2.4) rectangle (7.8, 3.9);
        \fill[black!60] (5.8, 3.6) rectangle (7.8, 3.9);
        \fill[white] (5.95, 3.75) circle (1.2pt); \fill[white] (6.1, 3.75) circle (1.2pt); \fill[white] (6.25, 3.75) circle (1.2pt);
        \fill[gray!20, draw=black!70, thick] (5.3, 1.8) rectangle (7.3, 3.3);
        \fill[black!60] (5.3, 3.0) rectangle (7.3, 3.3);
        \fill[white] (5.45, 3.15) circle (1.2pt); \fill[white] (5.6, 3.15) circle (1.2pt); \fill[white] (5.75, 3.15) circle (1.2pt);
        \fill[white, draw=black!40] (5.5, 2.0) rectangle (7.1, 2.8);
        \fill[gray!20, draw=black!70, thick] (4.8, 1.2) rectangle (6.8, 2.7);
        \fill[black!60] (4.8, 2.4) rectangle (6.8, 2.7);
        \fill[white] (4.95, 2.55) circle (1.2pt); \fill[white] (5.1, 2.55) circle (1.2pt); \fill[white] (5.25, 2.55) circle (1.2pt);
        \fill[white, draw=black!40] (5.0, 1.4) rectangle (5.8, 2.2);
        \fill[black!30] (5.6, 2.0) circle (2.5pt);
        \fill[black!50] (5.0, 1.4) -- (5.4, 1.9) -- (5.6, 1.6) -- (5.8, 1.8) -- (5.8, 1.4) -- cycle;
        \fill[black!30] (6.0, 2.0) rectangle (6.6, 2.1);
        \fill[black!30] (6.0, 1.7) rectangle (6.6, 1.8);
        \node[font=\sffamily] at (6.3, 4.4) {Web Content};
        \node[fit=(web_tl) (web_br)] (web_group) {};
    \end{scope}

    % --- D. Person Identification ---

    \begin{scope}[opacity=\opPiOne, transparency group]
        \node[inner_box_green] (pi_b1) at (12.5, 3.5) {Face Detection};
    \end{scope}
    
    \begin{scope}[opacity=\opPiTwo, transparency group]
        \node[inner_box_green] (pi_b2) at (12.5, 1.5) {Feature\\Extraction};
    \end{scope}
    
    \begin{scope}[opacity=\opPiThree, transparency group]
        \node[inner_box_green] (pi_b3) at (17.8, 3.5) {Face\\Verification};
    \end{scope}
    
    \begin{scope}[opacity=\opPiFour, transparency group]
        \node[inner_box_green] (pi_b4) at (17.8, 1.5) {Visualization};
    \end{scope}

    % PI Internal Arrows
    \begin{scope}[opacity=\opPiTwo, transparency group]
        \draw[green_arrow] (pi_b1) -- (pi_b2);
    \end{scope}
    \begin{scope}[opacity=\opPiThree, transparency group]
        \draw[green_arrow] (pi_b2.east) -- ++(0.6,0) |- (pi_b3.west);
    \end{scope}
    \begin{scope}[opacity=\opPiFour, transparency group]
        \draw[green_arrow] (pi_b3) -- (pi_b4);
    \end{scope}

    % =========================================================
    % 3. GLOBAL ROUTING ARROWS
    % =========================================================
    
    % Internal to Facial Feature Extractor (Data to CNN)
    % Visible when both Data and CNN are visible (Step 3 onwards)
    \begin{scope}[opacity=\opCnn, transparency group]
        \draw[gray_arrow] (data_group.east) -- (cnn_group.west);
    \end{scope}

    % Internal to Internet Archive (Logo to Web Content)
    % Visible when both Logo and Web are visible (Step 11 onwards)
    \begin{scope}[opacity=\opWeb, transparency group]
        \draw[gray_arrow] (ia_logo_group.east) -- (web_group.west);
    \end{scope}

    % Red Split Arrow: CNN to Feature Extractions
    % Visible when CNN, PD_b2, and PI_b2 are visible. 
    % CNN is 3, PD_b2 is 7, PI_b2 is 14. So visible from 14 onwards.
    \begin{scope}[opacity=\opPiTwo, transparency group]
        \coordinate (red_split) at (9.25, 10.5);
        \draw[red_arrow, -] (cnn_group.east) -- (red_split);
        \draw[red_arrow] (red_split) |- (pd_b2.west);
        \draw[red_arrow] (red_split) |- (pi_b2.west);
    \end{scope}

    % Orange Arrow: Web Content to Face Detection
    % Visible when Web (11) and PI_b1 (13) are visible. So from 13 onwards.
    \begin{scope}[opacity=\opPiOne, transparency group]
        \draw[orange_arrow] (web_group.east) -- (9.75, 2.5) |- (pi_b1.west);
    \end{scope}

    % Blue Arrow: Filtering to Face Verification
    % Visible when PD_b3 (8) and PI_b3 (15) are visible. So from 15 onwards.
    \begin{scope}[opacity=\opPiThree, transparency group]
        \draw[blue_arrow] (pd_b3.south) -- (16.05, 6.5) -- (21.2, 6.5) |- (pi_b3.east);
    \end{scope}

\end{tikzpicture}
}
\end{animateinline}
\end{document}